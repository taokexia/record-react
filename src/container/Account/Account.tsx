...
import { createForm, WrappedFormClass, WrappedFormMethods  } from 'rc-form';
import { postUserPassword } from '../../utils';
...
const Account: FunctionComponent<AccountProps> = (props) => {
  ...
  const submit = () => {
    ...
    props.form.validateFields(async (error, value) => {
      ...
      if (!error) {
        ...
        await postUserPassword({
          oldPassword: value.oldpass,
          password1: value.newpass,
          password2: value.newpass2
        })
        Toast.show('修改成功')
      }
    });
  ...
  return <>
    ...
    <div className={styles.account}>
      ...
      <Button className={styles.btn} block theme="primary" onClick={submit}>提交</Button>
    </div>
  </>
};

export default createForm()(Account as any);